...
def debug_quarterly_strategy(market_name: str, start: str, end: str):
    ...
    for i, (ts_date, row) in enumerate(market.data.iterrows()):
        ...
        expired_count = sum(
            1 for opt in portfolio.options if ts_date >= pd.Timestamp(opt.expiry)
        )
    ...
    print(
        f"Final return: {(portfolio.total_value(price, ts_date) / 1_000_000 - 1) * 100:.2f}%"
    )
    ...
    if exercise_log:
        ...
        if itm_exercises:
            ...
            for e in itm_exercises[:5]:
                print(
                    f"  {e['Date'].date()}: Strike ${e['Strike']:,.0f}, "
                    f"Price ${e['Price']:,.0f}, "
        ...
        print(
            "This explains the 0% benefit - options were purchased but never realized."
        )
