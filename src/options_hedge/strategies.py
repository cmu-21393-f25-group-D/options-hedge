...
def quarterly_protective_put_strategy(
    ...
    if last_action is None or (
        (current_date - last_action).days >= hedge_interval
    ):
        ...
        portfolio.buy_put(
            strike, premium, expiry, quantity=DEFAULT_OPTION_QUANTITY
        )
...
def vix_ladder_strategy(
    ...
    ladder_budget_allocations = params.get(
    ...
    if verbose:
        ...
        budget_calc = (
            V0 * 0.01 * (vix / 20.0) * max(1.0, beta)
        )
        print(f"VIX-Responsive Budget: ${budget_calc:,.2f}")
        vix_fac = vix / 20.0
        beta_fac = max(1.0, beta)
        print(
            f"  (Base 1% × VIX factor {vix_fac:.2f} "
            f"× Beta factor {beta_fac:.2f})"
        )
